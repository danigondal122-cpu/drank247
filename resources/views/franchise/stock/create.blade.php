      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">{{ (empty($row)==false)?'Update Stock':'Add New Stock'}}</h3>
             ...
             @csrf
             <input type="hidden" name="stock_id" id="stock_id" value="{{ (empty($row)==false)?$row->id:''}}">
             <div class="card-body col-sm-12 col-md-6">
              <div class="row">
                <div class="col-sm-12">
                  <div class="form-group">
                     <label for="first_name">*Product</label>
                     <select name="product_name" id="product_name" class="form-control select2" onchange="getCategory(this.value);">
                       <option value="">Select Product</option>
                       @foreach ($products as $product)
                       <option value="{{$product->id}}" {{ (empty($row)==false && ($product->id==$row->product_id))?'selected':''}}>{{$product->product_name}}</option>
                       @endforeach
                     </select>
                   </div>
                 ...
                 <div class="col-sm-12">
                   <div class="form-group">
                      ...
                      <select name="category_name" id="category_name" class="form-control select2">
                        ...
                        <option value="{{$category->id}}" {{ (empty($row)==false && ($category->id==$row->stock_category))?'selected':''}}>{{$category->category_name}}</option>
                        @endforeach
                      </select>
                    </div>
                ...
                <div class="row">
                  <div class="col-sm-12">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="current_stock" name="current_stock" value="{{ (empty($row)==false)?$row->stock_current:''}}">
                    </div>
                ...
                <div class="row">
                  <div class="col-sm-12">
                    <div class="form-group">
                       ...
                       <input type="text" class="form-control" id="min_stock" name="min_stock" value="{{ (empty($row)==false)?$row->stock_minimum:''}}">
                     </div>
...
<script>
    ...
    $.ajax({
        url: SITE_URL + 'franchise/stock/add',
        type: 'POST',
        data: data,
    ...
    $.ajax({
        url: SITE_URL + 'franchise/stock/getCategory',
        type: 'POST',
