    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          ...
          <input type="hidden" name="id" id="id" value="{{auth('franchise')->id()}}">
        </div>
        <!-- /.card-header -->
...
<script src="{{ asset('plugins/datatables-bs4/js/dataTables.bootstrap4.min.js') }}"></script>
  <script>
   ...
   table = $('#table').DataTable({
     ...
     "ajax": {
       "url": SITE_URL + "franchise/help/list",
       "type": "POST",
       "data": function(d) {
        d._type="1",
        d._id=$("#id").val();
     ...
     columns: [
        ...
        { data: 'message' },
        {
          "mRender": function ( data, type, row ) {
              html = "<a href='#' style='color:"+row.os_color +"''  class='orderstatus' id='status"+row.id+"' status="+row.order_status+"  data-type='select'  data-pk='"+row.id +"'  class='editable editable-click' data-original-title='' title=''>"+row.os_name +"</a>";
            return html;
          }
        },
        {
          "mRender": function ( data, type, row ) {
            ...
            let html = "<div class='btn-group float-left pr-2'><a href='javascript:;' onclick='deleteHelp("+row.id+")' class='btn btn-xs btn-danger btn-delete text-white'><i class='fa fa-trash-alt'></i></a></div>";
            return html;
          }
        },
     ...
     "initComplete": function( settings, json ) {
      $('.orderstatus').editable({
          ...
          source: [
            ...
            mode: 'popup',
            url: SITE_URL + 'franchise/help/updatestatus',
     ...
     let html = `
      ...
      @foreach ($deliverylist as $del)
        html += `<option value="{{ $del->id }}">{{ $del->dp_name }}</option>`;
      @endforeach
      html +=`</select>
    ...
    var dp_id = $('#dp_id').val();
    var new_url = SITE_URL + "franchise/help/list?dp_id="+dp_id;
  ...
  $(document).on('click', '.btn-reset', function() {
    ...
    table
      .search('')
      .columns().search('');
    var new_url = SITE_URL + "franchise/help/list";
    table.ajax.url(new_url).load();
  });
      ...
      $.confirm({
        ...
        buttons: {
          confirm: {
            ...
            action: function () {
              $.ajax({
                url: SITE_URL + 'franchise/help/delete',
                type: 'POST',
