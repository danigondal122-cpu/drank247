        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    {{-- <a href="{{ url('customer_service/deliveryperson/add') }}" class="btn btn-primary btn-sm float-right">Add Delivery Person</a> --}}
                </div>
                <!-- /.card-header -->
...
@section('pageJS')
    ...
    <script>
        ...
        table = $('#table').DataTable({
            ...
            "ajax": {
                "url": SITE_URL + "customer_service/deliveryperson/list",
                "type": "POST",
                "data": function(d) {
            ...
            columns: [
                ...
                // { data: 'area' },
                {
                    "mRender": function(data, type, row) {
                        ...
                        let html =
                            "<div class='btn-group float-left' style='margin-right: -14px;'><div class='custom-control custom-switch custom-switch-off-danger custom-switch-on-success'> <input type='checkbox' class='custom-control-input scheduleonoff' id='customSwitch" +
                            row.id + "' onchange='onoff(" + row.id + ");' " + $color +
                            " disabled><label class='custom-control-label' for='customSwitch" + row.id +
                            "'></label></div></div>";
                        html += "<div class='btn-group float-left pr-2'><a href='" + SITE_URL +
                            "customer_service/deliveryperson/edit/" + row.id +
                            "' class='btn btn-xs btn-secondary btn-edit text-white'><i class='fa fa-pencil-alt'></i></a></div>";
                        html +=
                            "<div class='btn-group float-left pr-2'><a href='javascript:;' onclick='getFranchises(" +
                            row.id +
                            ")'  class='btn btn-xs btn-primary  btn-view text-white'><i class='fas fa-eye'></i></a></div>";

                        html +=
                            "<div class='btn-group float-left pr-2'><a href='javascript:;' onclick='deleteDelivery(" +
                            row.id +
   ...
   <button class="btn btn-danger btn-sm float-right btn-reset ml-2">Reset</button>
     `;
        ...
        $(document).on('change', '#frs_id', function() {
            var frs_id = $(this).val();
            var new_url = SITE_URL + "customer_service/deliveryperson/list?frs_id=" + frs_id;
        ...
        $(document).on('click', '.btn-reset', function() {
            ...
            table
                .search('')
                .columns().search('');
            var new_url = SITE_URL + "customer_service/deliveryperson/list";
            table.ajax.url(new_url).load();
        });
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: {
                        ...
                        action: function() {
                            $.ajax({
                                url: SITE_URL + 'customer_service/deliveryperson/delete',
                                type: 'POST',
        ...
        function getFranchises(id) {
            $.ajax({
                url: SITE_URL + 'customer_service/deliveryperson/getFranchises',
                type: 'POST',
