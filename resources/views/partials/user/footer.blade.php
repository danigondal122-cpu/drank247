						...
						<div class="row justify-content-center px-5">
							<div class="col-md-10">
								<div class="row">
									...
									@if ($global['settings']?->time_schedule)
										<div class="col-md-6 col-sm-6 col-lg-4">
											<div class="widget information_links wow fadeIn footer_ul_div">
												<h4 class="widget-title" itemprop="headline">{{ __('messages.deliverytime') }}</h4>
												@foreach ($global['dayschedule'] as $schedule)
													@if ($schedule->is_checked)
														<div class="row">
															<div class="col-md-6 col-6" style="color:#e91362;line-height:30px;">{{ $schedule->day }}</div>
															<div class="col-md-6 col-6" style="line-height:30px;">
																{{ $schedule->time() }}
															</div>
														</div>
													@endif
													...
													<ul>
														<li> 
															<a> 
																...
																<span>{{ $global['settings']?->address }}</span>
															</a>
														</li>
														<li>
															<a href="tel:{{ $global['settings']?->contact_no }}">
																...
																<span>{{ $global['settings']?->contact_no }}</span>
															</a>
														</li>
														@if ($global['settings']?->email_show)
															<li>
																<a>
...
@if (Request::segment(1) != 'cart')
	<div id="CartButton" class="btn btn-primary mt-2">
		<a href='{{ url('/cart') }}'>
			...
			<span class="badge navbar-badge" id="cart_total_item_footer">
				{{ cart()->count() }}
			</span>
			&nbsp;&nbsp;Winkelmandje (<span id="final_amount_footer">€&nbsp;{{ cart()->payment('total') }}</span>)
		</a>
	</div>
@endif
