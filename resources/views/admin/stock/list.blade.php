        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ url('admin/stock/add') }}" class="btn btn-primary btn-sm float-right">Add Stock</a>
                </div>
                <!-- /.card-header -->
                ...
                // { data: 'stock_price' },
                {
                    "mRender": function(data, type, row) {
                        let html = "<div class='btn-group float-left pr-2'><a href='" + SITE_URL +
                            "admin/stock/edit/" + row.id +
                            "' class='btn btn-xs btn-secondary btn-edit text-white'><i class='fa fa-pencil-alt'></i></a></div>";
                        html +=
                            "<div class='btn-group float-left pr-2'><a href='javascript:;' onclick='deleteStock(" +
                            row.id +
   ...
   <button class="btn btn-danger btn-sm float-right btn-reset ml-2">Reset</button>
     `;
        ...
        $(document).on('change', '#frs_id', function() {
            var frs_id = $(this).val();
            var new_url = SITE_URL + "admin/stock/list?frs_id=" + frs_id;
        ...
        $(document).on('click', '.btn-reset', function() {
            ...
            table
                .search('')
                .columns().search('');
            var new_url = SITE_URL + "admin/stock/list";
            table.ajax.url(new_url).load();
        });
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: {
                        ...
                        action: function() {
                            $.ajax({
                                url: SITE_URL + 'admin/stock/delete',
                                type: 'POST',
