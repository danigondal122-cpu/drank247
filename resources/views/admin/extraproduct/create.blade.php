...
@section('header_content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">{{ empty($row) == false ? 'Update Product' : 'Add Extra Product' }}</h1>
                </div>
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ empty($row) == false ? 'Update Product' : 'Add Extra Product' }}</h3>
                </div>
                <form method="post" id="addExtraProduct" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="card-body col-sm-12 col-md-6">
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="price" name="price"
                                        ...
                                        value="{{ empty($row) == false ? $row->product_price : '' }}">
                                </div>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="vat_price" name="vat_price"
                                        value="{{ empty($row) == false ? $row->vat_price : '' }}" readonly>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label for="first_name">Order From</label>
                                    <select class="form-control" id="order_from" name="order_from">
                                        <option value="0"
                                            {{ empty($row) == false && $row->order_from == 0 ? 'selected' : '' }}>
                                            247AUTOSTOCK</option>
                                        <option value="1"
                                            {{ empty($row) == false && $row->order_from == 1 ? 'selected' : '' }}>
                                            247WINEHOUSE</option>
                                        <option value="2"
                                            {{ empty($row) == false && $row->order_from == 2 ? 'selected' : '' }}>
                                            247WAREHOUSE</option>
                                    </select>
                                </div>
                    ...
                    <div class="card-footer">
                        ...
                        <a href="{{ url('admin/extraproduct/list') }}" class="btn btn-secondary text-white"> <i
...
@section('pageJS')
    ...
    <script>
        ...
        $(document).on('submit', '#addExtraProduct', function(e) {
            e.preventDefault();
            loader_show();
            ...
            $('#addExtraProduct .is-invalid').removeClass('is-invalid');
            $('#addExtraProduct .text-danger').remove();
            $.ajax({
                url: SITE_URL + 'admin/extraproduct/add',
                type: 'POST',
                data: data,
                success: function(obj) {
